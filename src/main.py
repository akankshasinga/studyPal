...
if not GROQ_API_KEY:
    raise RuntimeError("Missing GROQ_API_KEY in environment. Add it to .env in the src folder.")
DEVICE = os.getenv('DEVICE', 'cpu')  # Default to 'cpu' if not set

working_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.dirname(working_dir)

subjects_list = ["C++", "Java", "Python"]

# Helper to setup vectorstore and chat_chain
def get_vector_db_path(chapter, subject):
    if chapter == "All Chapters":
        return f"{parent_dir}/vector_db/coding_{subject}_vector_db"
...
selected_subject = st.selectbox(
    label="Select a Subject from Coding",
...
if selected_subject:
    ...
    selected_chapter = st.selectbox(
        label=f"Select a Chapter from Coding - {selected_subject}",
